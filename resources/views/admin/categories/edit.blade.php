...
@section('content')
    <header
        ...
        <div>
            <h2 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Edit Kategori</h2>
            <p class="text-slate-500 dark:text-slate-400 font-sans text-sm">Perbarui nama kategori.</p>
        </div>
    ...
    <div class="flex-1 overflow-auto px-8 pb-8">
        <div
            ...
            <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="flex flex-col gap-6">
                @csrf
                @method('PUT')
                <div class="flex flex-col gap-2">
                    <label class="text-sm font-semibold text-slate-700 dark:text-slate-300 font-sans">Nama Kategori</label>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800 mt-2">
                    ...
                    <button type="submit"
                        class="px-6 py-2.5 rounded-lg bg-primary text-white hover:bg-primary-dark font-sans font-medium shadow-lg shadow-primary/30 transition-all">Perbarui</button>
                </div>
            </form>
        </div>
    </div>
